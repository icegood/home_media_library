...
func TestUserSettingsThumbBounds(t *testing.T) {
	f := setup(t)
	alice := login(t, f.handler, "alice")
	settingsURL := "/api/v1/settings"

	base := map[string]any{"theme": "light", "codec": "h264-aac-mp4", "zoom": 100,
		"dateFormat": "auto", "streamChunkSize": 10000, "defaultThumbImage": "mountains",
		"defaultThumbVideo": "mountains", "defaultThumbFolder": "mountains"}

	put := func(patch map[string]any) *httptest.ResponseRecorder {
		body := map[string]any{}
		for k, v := range base {
			body[k] = v
		}
		...
		return request(f.handler, http.MethodPut, settingsURL, alice, encoded)
	}
	get := func() domain.UserSettings {
		refetched := request(f.handler, http.MethodGet, settingsURL, alice, nil)
		var updated domain.UserSettings
		if err := json.Unmarshal(refetched.Body.Bytes(), &updated); err != nil {
			t.Fatal(err)
	...
	if updated := get(); updated.ThumbMin != 90 || updated.ThumbMax != 0 {
		t.Fatalf("default thumb bounds = %d/%d, want 90/0", updated.ThumbMin, updated.ThumbMax)
	}

	if got := put(map[string]any{"thumbMin": 120, "thumbMax": 120}).Code; got != http.StatusOK {
		t.Fatalf("min=max put status = %d", got)
	}
	if updated := get(); updated.ThumbMin != 120 || updated.ThumbMax != 120 {
		t.Fatalf("min=max saved as %d/%d, want 120/120", updated.ThumbMin, updated.ThumbMax)
	}

	if got := put(map[string]any{"thumbMin": 90, "thumbMax": 200}).Code; got != http.StatusOK {
		t.Fatalf("capped put status = %d", got)
	}
	if updated := get(); updated.ThumbMax != 200 {
		t.Fatalf("capped max saved as %d, want 200", updated.ThumbMax)
	}

	if got := put(map[string]any{"thumbMin": 220}).Code; got != http.StatusOK {
		t.Fatalf("min-only put status = %d", got)
	}
	if updated := get(); updated.ThumbMin != 220 || updated.ThumbMax != 0 {
		t.Fatalf("min-only saved as %d/%d, want 220/0", updated.ThumbMin, updated.ThumbMax)
	}

	if got := put(map[string]any{"thumbMax": 150, "thumbMin": 200}).Code; got != http.StatusBadRequest {
		t.Fatalf("max below min status = %d, want 400", got)
	}
	if got := put(map[string]any{"thumbMin": 39}).Code; got != http.StatusBadRequest {
		t.Fatalf("min 39 status = %d, want 400", got)
	}
	if got := put(map[string]any{"thumbMax": 641}).Code; got != http.StatusBadRequest {
		t.Fatalf("max 641 status = %d, want 400", got)
	}

	if got := put(map[string]any{"mapThumbMin": 130, "mapThumbMax": 130}).Code; got != http.StatusOK {
		t.Fatalf("legacy-key put status = %d", got)
	}
	if updated := get(); updated.ThumbMin != 130 || updated.ThumbMax != 130 {
		t.Fatalf("legacy keys saved as %d/%d, want 130/130", updated.ThumbMin, updated.ThumbMax)
	}
}
