...
func mediaNeighbors(ctx context.Context, db *sql.DB, enrich func(context.Context, []domain.Media, int) error, driver string, userID, libraryID, folderID, anchorID int, sort, kind, gps string, before, after int) (domain.MediaNeighbors, error) {
	...
	_ = enrich(ctx, result.Before, 0)
	_ = enrich(ctx, result.After, 0)
	enriched := []domain.Media{result.Anchor}
	_ = enrich(ctx, enriched, 0)
