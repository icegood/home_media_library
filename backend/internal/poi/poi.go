...
// perOverpassTimeout caps a single mirror attempt. Five seconds covers TLS
// handshake + query + response transfer; anything slower is effectively down.
const perOverpassTimeout = 5 * time.Second
...
// buildOverpassQuery assembles the Overpass QL query using the compact `nwr`
// (node-way-relation) shorthand which is significantly faster than unioning
// three separate element queries.
func buildOverpassQuery(bbox BBox, categories []CategoryID) (string, error) {
	...
	for _, cat := range categories {
		for _, cond := range overpassConditions(cat) {
			...
			parts = append(parts, fmt.Sprintf(`nwr[%s](%f,%f,%f,%f);`,
	...
	return fmt.Sprintf(`[out:json][timeout:3];%sout center 200;`,
