...
function mediaRow(overrides:Partial<Media> = {}):Media {
  return {id:100, folderId:20, relativePath:"one.jpg", name:"one.jpg", kind:"image", mimeType:"image/jpeg", size:10, metadata:{}, gps:"", takenAt:"", notes:"", ...overrides};
}
...
function createMockApi() {
  ...
  function resetState() {
    ...
    state = {
      ...
      userSettings:{theme:"light", codec:"h264-aac-mp4", zoom:100, dateFormat:"auto", language:"auto", streamChunkSize:10000, defaultThumbImage:"mountains", defaultThumbVideo:"mountains", defaultThumbFolder:"mountains", mapTileProviderLight:"osm", mapTileProviderDark:"osm", mapMaxZoom:19, mapTileProviders:{carto:{apiKey:""}}, poiProviderLight:"overpass", poiProviderDark:"overpass", poiProviders:{overpass:{endpoint:""}}, thumbMin:90, thumbMax:0},
      adminSettings:{
  ...
  const api = {
    ...
    updateMediaDetails: vi.fn(async (id:number, input:{name:string; gps:string|null; takenAt:string|null; notes?:string|null}) => {
      const row = state.media.get(Number(id));
      if (!row) throw new Error(`media ${id} not found`);
      Object.assign(row, {name:input.name, gps:input.gps ?? "", takenAt:input.takenAt ?? "", notes:input.notes ?? ""});
      return JSON.parse(JSON.stringify(row));
    }),
